...
var $nowDirectory = $nowPathname.substring(0, $nowPathname.lastIndexOf('/')).slice(1);
var $listOfNoHeader = ['liw-beginner', 'paff-alpha']; // 固定ヘッダーがないLPのスクロール制御用
...
$('a[href^="#"].anker').on('click', function () {
    ...
    if (window.matchMedia('(min-width: 768px)').matches) {
        var position = $listOfNoHeader.includes($nowDirectory) ? target.offset().top : target.offset().top - 80;
...
function hashAnker() {
  ...
  var idName = urlHash.substr(0, urlHash.indexOf('_anker'));
  var animeSpeed = 900;

  if (urlHash == idName + '_anker') {
...
function loading() {
    $form.submit(function (e) {
        ...
        $('body').append(
            '  <div id="loading"><div class="inner"><div class="dot-spin"></div><p class="loding-text">ただいま処理中です...<br>完了ページが表示されるまでお待ちください。</p></div></div>'
        );
    });
}
function loading_safari() {
    $form.submit(function (e) {
        ...
        $('body').append(
            '  <div id="loading"><div class="inner"><p class="loding-text">ただいま処理中です...<br>完了ページが表示されるまでお待ちください。</p></div></div>'
        );
    });
}
...
if ($video.length) {
    ...
    $(window).on('scroll resize', function () {
        ...
        if (
            !$video[0].paused &&
            (windowScrollTop + windowInnerHeight < videoTop ||
                windowScrollTop > videoTop + videoHeight)
        ) {
            $video[0].pause();
        }
    });
...
if ($('#js-reservation').length) {
    ...
    $('#Userseminar').change(function () {
        if ($(this).val() == '優先案内 [東京]') {
            $('#UserItemForm').attr(
                'action',
                'https://go.tapp-co.jp/l/649443/2020-03-05/crjzk'
            );
            $submit_btn.text('優先案内を希望する');
        } else if ($(this).val() == '優先案内 [大阪]') {
            $('#UserItemForm').attr(
                'action',
                'https://go.tapp-co.jp/l/649443/2020-03-05/crjzw'
            );
            $submit_btn.text('優先案内を希望する');
        } else if ($(this).val() == '優先案内 [名古屋]') {
            $('#UserItemForm').attr(
                'action',
                'https://go.tapp-co.jp/l/649443/2020-03-05/crk11'
            );
            $submit_btn.text('優先案内を希望する');
        } else if ($(this).val() == '優先案内 [福岡]') {
            $('#UserItemForm').attr(
                'action',
                'https://go.tapp-co.jp/l/649443/2020-03-05/crk13'
            );
            $submit_btn.text('優先案内を希望する');
        } else {
